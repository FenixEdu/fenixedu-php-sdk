...
class FenixEdu {
    ...
    public function __construct($config, $tokenHolder = NULL) {
        ...
        $this->connector = new FenixEduConnector($config, $tokenHolder);
        $this->services = new FenixEduServices($this->connector);
        $this->root = NULL;
    }
    ...
    /** Get an interface for all of FenixEDU's endpoints.
     * Refrain from using this, unless you only intend to use one or two
     * endpoints or are really determined to make your code hard to maintain.
     */
    public function getServices() {
        return $this->services;
    }
    
    /** Get the domain root. This provides a much cleaner, object-oriented 
     * approach to using the API.
     */
    public function getDomainRoot() {
        require_once("domain/DomainRoot.php");
        if($this->root == NULL) $this->root = new DomainRoot($this->services);
        return $root;
    }
}
...
class FenixEduServices {
	...
	public function getCourse($id) {
		return $this->connector->get("courses/" . $id);
	}
	...
	public function getDegree($id, $academicTerm = NULL) {
		$query = $academicTerm !== NULL ? "?academicTerm=" . $academicTerm : "";
		return $this->connector->get("degrees/" . $id . $query);
	}
	 ...
	 */
	public function getPersonCourses($id, $academicTerm = NULL) {
		$query = $academicTerm !== NULL ? "?academicTerm=" . $academicTerm : "";
